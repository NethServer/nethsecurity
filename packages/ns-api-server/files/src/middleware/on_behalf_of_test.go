...
// TestOnBehalfOfClaim: the claim is stored only when set, so regular logins get unchanged tokens.
...
// TestOnBehalfOfLogSuffix: the operator is reported next to the machine account, without displacing the client IP banIP extracts.
